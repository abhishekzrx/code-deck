   ...
   <>
    ...
    <div className='px-6 py-4 mb-8 flex flex-col items-center justify-center gap-6'>
       ...
       className='border-[.5px] text-sm border-gray-50 rounded-lg shadow-sm p-2 w-full'
        type='text' onChange={(e)=>setFolderTitle(e.target.value)}  value={folderTitle} placeholder='Please Enter Folder Title'/>
        <button 
        onClick={()=>{
          addFolder(folderTitle);
          closeModal();
        }}
         className='p-3 w-36 text-black bg-white rounded-lg font-semibold border-[.5px] border-gray-50 shadow-lg'>Create Folder</button>
    </div>
   </>
  )
}
